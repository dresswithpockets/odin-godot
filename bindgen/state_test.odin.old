...
state_type_map_contains_every_referred_type :: proc(t: ^testing.T) {
    ...
    for builtin_class in api.builtin_classes {
        for constant in builtin_class.constants {
            testing.expectf(
                t,
                new_state_godot_type_in_map(state, constant.type),
                "Expected '%v' in all_types map",
                constant.type,
            )
        }
        ...
        for constructor in builtin_class.constructors {
            for argument in constructor.arguments {
                testing.expectf(
                    t,
                    new_state_godot_type_in_map(state, argument.type),
                    "Expected '%v' in all_types map",
                    argument.type,
                )
            }
        ...
        if indexing_return_type, ok := builtin_class.indexing_return_type.(string); ok {
            testing.expectf(
                t,
                new_state_godot_type_in_map(state, indexing_return_type),
                "Expected '%v' in all_types map",
        ...
        for member in builtin_class.members {
            testing.expectf(
                t,
                new_state_godot_type_in_map(state, member.type),
                "Expected '%v' in all_types map",
                member.type,
            )
        }
        ...
        for method in builtin_class.methods {
            ...
            if return_type, ok := method.return_type.(string); ok {
                testing.expectf(
                    t,
                    new_state_godot_type_in_map(state, return_type),
                    "Expected '%v' in all_types map",
                    return_type,
                )
            }
        ...
        for operator in builtin_class.operators {
            testing.expectf(
                t,
                new_state_godot_type_in_map(state, operator.return_type),
                "Expected '%v' in all_types map",
            ...
            if right_type, ok := operator.right_type.(string); ok {
                testing.expectf(
                    t,
                    new_state_godot_type_in_map(state, right_type),
                    "Expected '%v' in all_types map",
                    right_type,
                )
            }
    ...
    for class in api.classes {
        ...
        for method in class.methods {
            ...
            if return_value, ok := method.return_value.(ApiClassMethodReturnValue); ok {
                testing.expectf(
                    t,
                    new_state_godot_type_in_map(state, return_value.type),
                    "Expected '%v' in all_types map",
                    return_value.type,
                )
                if meta, ok := return_value.meta.(string); ok {
                    testing.expectf(
                        t,
                        new_state_godot_type_in_map(state, meta),
                        "Expected '%v' in all_types map",
                        meta,
                    )
                }
